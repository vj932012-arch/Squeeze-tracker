...
def render_ticker_dashboard(ticker: str, processed_df: pd.DataFrame, adx_len: int, adx_thresh: float, spread_width: float):
    ...
    if spread_info:
        ...
        st.write(
            ...
            "- TTM Squeeze: Fired with aligned momentum"
        )
    ...
    fig = make_subplots(
        ...
        subplot_titles=(f"{ticker} Candlesticks", "TTM Squeeze Momentum", "Relative Volume (RVOL)", "ADX & DMI (Trend)"),
    )
    ...
    # Row 2: TTM Squeeze Histogram & Dots
    if "histogram_col" in plot_df.columns:
        histogram_colors = []
        for i in range(len(plot_df)):
            val = plot_df['histogram_col'].iloc[i]
            prev_val = plot_df['histogram_col'].iloc[i-1] if i > 0 else 0
            if val > 0:
                color = "#00b0ff" if val > prev_val else "#0d47a1"  
            else:
                color = "#ff5252" if val < prev_val else "#ffeb3b"  
            histogram_colors.append(color)

        fig.add_trace(go.Bar(x=plot_df.index, y=plot_df['histogram_col'], marker_color=histogram_colors, name="TTM Momentum"), row=2, col=1)
        
        squeeze_on = plot_df[plot_df['SQZ_ON'] == 1]
        squeeze_fired = plot_df[plot_df['SQZ_OFF'] == 1]
        
        fig.add_trace(go.Scatter(x=squeeze_on.index, y=[0]*len(squeeze_on), mode="markers", marker=dict(color="#ff5252", size=5), name="Squeeze ON"), row=2, col=1)
        fig.add_trace(go.Scatter(x=squeeze_fired.index, y=[0]*len(squeeze_fired), mode="markers", marker=dict(color="#00e676", size=5), name="Squeeze FIRED"), row=2, col=1)

    # Row 3: RVOL
    fig.add_trace(go.Bar(x=plot_df.index, y=plot_df["rvol"], name="RVOL", marker_color=np.where(plot_df["rvol"] >= 1.3, "#00e676", "#78909c")), row=3, col=1)
    fig.add_hline(y=1.3, line_dash="dot", line_color="#ffca28", row=3, col=1)
    ...
    if adx_col in plot_df.columns:
        fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df[adx_col], line=dict(color="#FFD700", width=2), name="ADX"), row=4, col=1)
        fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df[dmp_col], line=dict(color="#00e676", width=1.2), name="+DI"), row=4, col=1)
        fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df[dmn_col], line=dict(color="#ff5252", width=1.2), name="-DI"), row=4, col=1)
        fig.add_hline(y=adx_thresh, line_dash="dot", line_color="#b0bec5", row=4, col=1, annotation_text=f"Threshold ({adx_thresh})", annotation_position="bottom right")
...
st.caption("Multi-factor signal scanner analyzing normalized EMA deltas, VWAP displacement, RVOL expansion, ADX trend strength, and TTM Squeeze breakouts.")
